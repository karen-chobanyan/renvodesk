...
insert into public.project_files(organization_id,project_id,id,original_name,mime_type,size_bytes) values(current_setting('test.org_a')::uuid,'cccccccc-cccc-4ccc-8ccc-cccccccccccc','dddddddd-dddd-4ddd-8ddd-dddddddddddd','Plan.pdf','application/pdf',100);
select set_config('test.file_key',(select object_key from public.project_files limit 1),true);
do $$ begin
 ...
 begin update public.project_files set original_name='Rename'; raise exception 'Metadata mutation'; exception when insufficient_privilege then null; end;
 begin insert into storage.objects(bucket_id,name) values('project-files','unreserved'); raise exception 'Unreserved upload'; exception when insufficient_privilege then null; end;
end $$;
insert into storage.objects(bucket_id,name,metadata) values('project-files',current_setting('test.file_key'),'{"size":100,"mimetype":"application/pdf"}');
update public.project_files set state='ready';
do $$ begin
 if (select state from public.project_files limit 1)<>'ready' then raise exception 'Finalize failed'; end if;
 if not exists(select 1 from storage.objects where name=current_setting('test.file_key')) then raise exception 'Own storage read failed'; end if;
 if private.can_upload_project_file(current_setting('test.file_key')) then raise exception 'Ready overwrite allowed'; end if;
 ...
 if private.can_upload_project_file(current_setting('test.file_key')) then raise exception 'Upload during delete allowed'; end if;
end $$;
select set_config('request.jwt.claims','{"sub":"22222222-2222-4222-8222-222222222222","role":"authenticated"}',true);
do $$ begin
 if exists(select 1 from public.project_files) or exists(select 1 from storage.objects where name=current_setting('test.file_key')) then raise exception 'File tenant leak'; end if;
 update public.project_files set state='deleted'; if found then raise exception 'Cross tenant mutation'; end if;
 if private.can_upload_project_file(current_setting('test.file_key')) then raise exception 'Cross tenant upload'; end if;
...
do $$ begin
 begin perform * from public.project_files; raise exception 'Anonymous file access'; exception when insufficient_privilege then null; end;
