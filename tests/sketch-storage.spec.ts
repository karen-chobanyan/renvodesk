...
test("saved sketch opens in place, restores history and restricts members", async ({
  page,
}) => {
  const user = {
    id: uid,
  ...
  const path = `/workspace/${org}/projects/${project}?tab=documents#project-sketches`;
  ...
  expect(
    await page.evaluate(() => document.body.dataset.sketchPageMarker),
  ).toBe("kept");
  expect(new URL(page.url()).pathname).toBe(
    `/workspace/${org}/projects/${project}`,
  );
  await expect(page.getByRole("dialog", { name: "Site plan" })).toBeVisible();
  ...
  if (test.info().project.name === "mobile") {
    await page
      .locator(".sketch-viewer-controls")
      .getByRole("button", { name: "Historique" })
      .click();
    const stage = await page.locator(".sketch-viewer-stage").boundingBox();
    const panel = await page.locator(".sketch-history").boundingBox();
    expect(stage && panel && panel.y >= stage.y + stage.height - 1).toBe(true);
  } else {
    const stage = await page.locator(".sketch-viewer-stage").boundingBox();
    const panel = await page.locator(".sketch-history").boundingBox();
    expect(stage && panel && panel.x + panel.width <= stage.x + 1).toBe(true);
  }
  await expect(
    page.getByRole("button", { name: "Version 2 · Site plan", exact: true }),
  ).toBeVisible();
  await page
    .getByRole("button", { name: "Version 1 · Site plan", exact: true })
    .click();
  expect(
    await page.evaluate(() => document.body.dataset.sketchPageMarker),
  ).toBe("kept");
  ...
  if (
    ...
    !(await page.locator(".sketch-history").isVisible())
  )
    await page
      .locator(".sketch-viewer-controls")
      .getByRole("button", { name: "Historique" })
      .click();
  await page
    .getByRole("button", { name: "Utiliser cette version", exact: true })
    .click();
  ...
  await page.locator(".sketch-dialog").screenshot({
    path: `/private/tmp/renvo-connected-sketch-${test.info().project.name}.png`,
  });
  await page.getByRole("button", { name: "Retour au projet" }).click();
  await expect(page.getByRole("dialog", { name: "Site plan" })).toHaveCount(0);
  expect(
    await page.evaluate(() => document.body.dataset.sketchPageMarker),
  ).toBe("kept");
  await page
    .locator(".project-tabs")
    .getByRole("link", { name: "Vue d’ensemble" })
    .click();
  await page.locator(".project-sketch-preview-open").click();
  await expect(page.getByRole("dialog", { name: "Site plan" })).toBeVisible();
  expect(
    await page.evaluate(() => document.body.dataset.sketchPageMarker),
  ).toBe("kept");
  ...
  await expect(page.getByRole("status")).toHaveText("Consultation uniquement");
  await expect(
    page.locator(".sketch-dialog").getByLabel("Nom du croquis"),
  ...
  await expect(
    page
      .locator(".sketch-dialog")
      .getByLabel("Nom du croquis", { exact: true }),
  ).toHaveValue("New kitchen");
  await expect(page.getByRole("status")).toHaveText("Enregistré");
  await page.goto(`/workspace/${org}/projects/${project}/sketches/${sk.id}`);
  await expect(page.getByRole("dialog", { name: "New kitchen" })).toBeVisible();
});
